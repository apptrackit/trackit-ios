...
// MARK: - Stats History Manager with New Sync Architecture
...
class StatsHistoryManager: ObservableObject {
    ...
    private let metricSyncManager = NewSyncManager.shared
    ...
    private init() {
        ...
        // Attempt migration from old system on first launch
        if entries.isEmpty {
    ...
    private func recalculateAllDerivedValues() {
        logger.info("Legacy recalculation method called - triggering refresh")
        loadEntries()
    }
}
